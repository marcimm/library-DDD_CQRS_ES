...
namespace MMM.Library.Services.AspNetWebApi.V2
{
    ...
    public class AuthController : ApiBaseController
    {
        ...
        public AuthController(ILogger logger, IIdentityService identityService,
        ...
        public async Task<IActionResult> NewUser(UserRegistrationViewModel userRegistration)
        {
            ...
            if (result)
            {
                ...
                _logger.Info(DateTime.Now.ToLongDateString() + " - Usuário criado com sucesso: " + userRegistration.Email);

                return CustomResponse(responseNewUser);
            }

            _logger.Error(DateTime.Now.ToLongDateString() + " - Falha ao criar Usuário: " + userRegistration.Email);
        ...
        public async Task<IActionResult> Login(UserLoginViewModel loginUser)
        {
            ...
            if (result)
            {
                ...
                _logger.Info(DateTime.Now.ToLongDateString() + " - Login realizado com sucesso para usuário: " + loginUser.Email);

                return CustomResponse(responseLogin);
            }

            _logger.Info(DateTime.Now.ToLongDateString() + " - Tentativa de Login falhou para usuário: " + loginUser.Email);
